...
UNIT = 100  # 픽셀 수
HEIGHT = 16  # 그리드월드 세로  Have to be modified
WIDTH = 9  # 그리드월드 가로  Have to be modified
TRANSITION_PROB = 1
POSSIBLE_ACTIONS = [0, 1, 2, 3]  # 상, 하, 좌, 우  Have to be modified
ACTIONS = [(-1, 0), (1, 0), (0, -1), (0, 1)]  # 좌표로 나타낸 행동  Have to be modified
REWARDS = []
...
class GraphicDisplay(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        # 그리드 생성
        for col in range(0, WIDTH * UNIT, UNIT):  # 0~400 by 80  Have to be modified
            x0, y0, x1, y1 = col, 0, col, HEIGHT * UNIT
            canvas.create_line(x0, y0, x1, y1)
        for row in range(0, HEIGHT * UNIT, UNIT):  # 0~400 by 80  Have to be modified
...
class Env:
    def __init__(self):
        ...
        # self.reward[2][2] = 1  # (2,2) 좌표 동그라미 위치에 보상 1 # Need to be modified
        # self.reward[1][2] = -1  # (1,2) 좌표 세모 위치에 보상 -1 # Need to be modified
        # self.reward[2][1] = -1  # (2,1) 좌표 세모 위치에 보상 -1 # Need to be modified
        self.all_state = []
    ...
    # def rgb2return_value(self, state, action): # Need to be modified
